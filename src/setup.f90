SUBROUTINE SETUP()
  ...
  DO z=1,nzmet
     DO t=1,nage
        IF (ssp_type.EQ.'vcj') THEN
           OPEN(22,FILE=TRIM(ALF_HOME)//'/infiles/VCJ_v7s_mcut0.08_'//&
                chart(t)//'_Z'//charz(z)//'.ssp.imf_varydoublex.s100',&
                STATUS='OLD',iostat=stat,ACTION='READ')
        ELSE
           OPEN(22,FILE=TRIM(ALF_HOME)//'/infiles/CvD_v7s_mcut0.08_'//&
  ...
  IF (imf_type.EQ.3) THEN 
     DO z=1,nzmet3
        DO m=1,nmcut
           DO t=1,nage
              OPEN(22,FILE=TRIM(ALF_HOME)//'/infiles/VCJ_v7s_mcut'//&
     ...
     DO z=1,nzmet
        DO t=1,nage
           IF (nonpimf_alpha.EQ.0.0) THEN
              OPEN(22,FILE=TRIM(ALF_HOME)//'/infiles/VCJ_v7s_'//&
