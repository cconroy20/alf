SUBROUTINE SET_WAVE_LIMITS(file)
  ...
  IF (file(1:4).EQ.'sdss'.OR.file(1:3).EQ.'ucd'&
       .OR.file(1:4).EQ.'boss'.OR.file(1:5).EQ.'manga') THEN
     l1 = (/0.40,0.47,0.58,0.80/) * 1E4
     l2 = (/0.47,0.58,0.64,0.88/) * 1E4
  ELSE IF (file(1:1).EQ.'n'.OR.file(1:3).EQ.'m32'.OR.&
       file(1:1).EQ.'b'.OR.file(1:5).EQ.'deep2') THEN
  ...
  ELSE IF (file(1:4).EQ.'goat') THEN
     l1  = (/0.40,0.47,0.56,0.8/) * 1E4
     l2  = (/0.47,0.56,0.68,0.885/) * 1E4
